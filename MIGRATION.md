...
### 0-5. Cursor가 “oom”으로 창이 죽을 때 (같은 작업을 여러 번 시도하게 될 때)
- **원인**: 대개 **메모리 부족(Out of Memory)** — 긴 채팅 + 큰 파일 여러 개 @ + 에디터 안에서 `npm run build` 등이 **한꺼번에** 겹치면 발생하기 쉽습니다. 코드가 틀려서라기보다 **에디터/세션 부하**에 가깝습니다.
- **안정적인 습관**: **새 채팅**으로 단계 나누기 · **`@MIGRATION.md`만** 붙이고 거대 파일은 필요할 때만 하나씩 · **빌드는 Cursor 밖 터미널**에서 · 안 쓰는 **탭 정리**. 상세는 **`.cursor/rules/project-handoff.mdc`** 마지막 불릿 참고.
...
**UI/UX (현재)**
- **태그로 보기**: `LogTagFilterRow` 칩만(긴 안내 문구 없음). **이번 글 태그**: 다음 로그에 붙는 `place_slug` 선택
...
- **홈 UX:** `feedFilterHint` 안내 문구 제거(피드 필터는 칩만).
...
*마지막 업데이트: 2026-04-02 — `SearchTabPanel` 분리; 피드 필터 힌트 제거; 설정 아이콘·스티키 헤더; DEPLOY 댓글 RLS §8·가계부 §7; `usePrefersReducedMotion` sync; 홈 페이드·칩 스냅; 가계부 지출·분류 카드 UX.*
