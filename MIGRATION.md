...
- **주요 파일**: `src/app/HomeClient.tsx` (메인 UI·상태), `src/lib/logTags.ts` (슬러그·필터), `src/components/home/LogTagFilterRow.tsx`, `LogFeed.tsx`, `CommentSheet.tsx` (댓글 하단 시트), `StickerPickerSheet.tsx` (로그 스티커 선택 시트), `src/app/globals.css`, `src/app/translations.ts`
