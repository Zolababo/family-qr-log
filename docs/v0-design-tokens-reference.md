...
> **상태:** 아래 스크랩 값은 **팔레트 참고용**. 제품 UI 색은 바꾸지 않고, `globals.css` **§5** 에서 `--v0-*` 이름으로 **기존 `--accent`·`--text-*` 등에 연결**해 두었음(시각 동일). 코럴/세이지 등으로 **실제 치환**할 때는 §5와 본 문서를 함께 검토하고, **고대비·접근성 글자**와 충돌 여부를 확인할 것.
...
v0.dev UI 제안 스크랩. 실제 제품 토큰은 `src/app/globals.css`(§1~4) 및 `HomeClient` 테마 객체를 우선한다. **`--v0-*` 별칭**은 `globals.css` §5.
